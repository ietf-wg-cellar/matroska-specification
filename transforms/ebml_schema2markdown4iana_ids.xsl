...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:str="http://exslt.org/strings" xmlns:ebml="urn:ietf:rfc:8794" >
  ...
  <xsl:template match="ebml:EBMLSchema">
    ...
    <xsl:text>----------:|:------------------------|:-------------------------------------------&#xa;</xsl:text>
    <xsl:apply-templates select="//ebml:element[contains(@path,'\Segment') and (not(@minver) or not(@added)) and string-length(@id)=4]">
    ...
    <xsl:apply-templates select="//ebml:element[contains(@path,'\Segment') and (not(@minver) or not(@added)) and string-length(@id)=6]">
      <xsl:sort select="@id" order="ascending" />
    </xsl:apply-templates>
    <xsl:call-template name="GenerateReserved">
    ...
    <xsl:apply-templates select="//ebml:element[contains(@path,'\Segment') and (not(@minver) or not(@added)) and string-length(@id)=8]">
      <xsl:sort select="@id" order="ascending" />
    </xsl:apply-templates>
    <xsl:call-template name="GenerateReserved">
    ...
    <xsl:apply-templates select="//ebml:element[contains(@path,'\Segment') and (not(@minver) or not(@added)) and string-length(@id)=10]">
      <xsl:sort select="@id" order="ascending" />
    </xsl:apply-templates>
    <xsl:call-template name="GenerateReserved">
